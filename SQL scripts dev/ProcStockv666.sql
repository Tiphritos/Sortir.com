...
BEGIN
    ...
    DECLARE sortiesOuvToClo CURSOR FOR
        ...
        WHERE date_cloture <= current_date AND etats_no_etat_id = 2;
    DECLARE sortiesCloToEC CURSOR FOR
        SELECT id FROM sortie
        WHERE date_debut <= current_date AND etats_no_etat_id = 3;
    DECLARE sortiesEcToPas CURSOR FOR
        SELECT id FROM sortie
        WHERE DATE_ADD(date_debut, INTERVAL duree MINUTE) <= current_date AND etats_no_etat_id = 4;
    DECLARE sortiesPasToArc CURSOR FOR
        SELECT id FROM sortie
        WHERE DATE_ADD(DATE_ADD(date_debut, INTERVAL duree MINUTE), INTERVAL 1 MONTH) <= current_date
            AND (etats_no_etat_id = 5 OR etats_no_etat_id = 6);

    #Changer l'etat de Ouverte � Cl�tur�e
    begin
    ...
    #Changer l'etat de Cl�tur�e � En cours
    begin
        DECLARE CONTINUE HANDLER
    ...
    #Changer l'etat de En Cours � Cl�tur�e
    begin
        DECLARE CONTINUE HANDLER
    ...
    #Changer l'etat de Clotur�e � Archiv�e
    begin
        DECLARE CONTINUE HANDLER
